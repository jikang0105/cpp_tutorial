...
int main() {
	...
	while (true) {
		...
		// cin은 만약 오른쪽에 int변수가 오면 정수를 입력해야 한다. 오른쪽에 오는 변수 타입에
		...
		// 먼저 에러를 체크한다. cin.fail()을 했을때 입력 에러가 발생했을 경우 true를 
		// 반환한다.
		if (cin.fail()) {
			// 에러버퍼를 비워준다.
			cin.clear();
			...
			// 입력버퍼에 \n이 남아있으므로 입력버퍼를 검색하여 \n을 지워준다.
			// 첫번째에는 검색하고자 하는 버퍼 크기를 지정한다. 넉넉하게 1024바이트 정도
			// 지정해주었다. 2번째는 찾고자 하는 문자를 넣어준다.
