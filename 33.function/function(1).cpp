...
인자 : 해당 함수를 사용하 때 넘겨줄 값이 있을 경우 인자를 활용한다. 인자는 있을수도
없을수도 있다.

해당 함수를 호출하면 코드블럭 안의 코드가 동작이 된다.
*/

// 아래 함수처럼 결과를 반환해주는 함수 형태를 Call by Value라고 한다.
int Sum(int a, int b) {
	// return 키워드를 이용해서 이 함수의 반환타입에 맞는 값을 반환해야 한다.
	return a + b;
}

// 함수는 단순히 만들어 놓기만 해서는 아무런 동작도 하지 않는다. 함수의 기능을 동작시키기
// 위해서는 반드시 main 함수 안에서 해당 함수를 호출해 주어야 한다.
void OutputText() {
	cout << "OutputText Function!" << endl;
}

// 아래 함수처럼 인자를 넘겨받게 도리 경우 이 인자는 이 함수 코드블럭의 지역에
...
// 함수의 디폴트인자 : 인자느 기본적으로 값을 넘겨받게 설계되어 있다. 그런데 인자에
// 기본값을 대입해두면 인자를 넘겨주지 않을 경우 기본 설정한 값이 되고 인자를 넘겨줄 경우
// 넘겨받은 값으로 인자를 설정하는 기능이다.
// 디폴트 인자는 가장 오른쪽 부터 지정해 주어야 한다.
...
오버로딩의 조건 : 함수이름이 무조건 같아야 한다. 단, 인자의 개수 혹은 타입이 달라야 
오버로딩이 성립이 된다. 반호나타입은 오버로딩에 영향을 주지 않는다.
*/
...
int main() {
	...
	// 함수를 호출할때는 함수명(인자로 점겨줄 값); 의 형태로 호출을 하게 된다.
	// 아래철머 호출하면 a에 10, b에 20이 전달되어 두 값을 더한 30이 반환된다.
	cout << Sum(10, 20) << endl;
	...
	// iNum1은 위의 코드블럭 안에 존재하는 변수이므로 저 코드블럭 안에서만 사용 할 수 있다.
	//cout << iNum1 << endl;
